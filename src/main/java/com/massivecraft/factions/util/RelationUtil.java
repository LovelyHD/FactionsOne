...
public class RelationUtil {
	public static String describeThatToMe(RelationParticipator that,
			RelationParticipator me, boolean ucfirst) {
		String ret = "";
		...
		// if (myFaction == null) return thatFaction.getTag(); // no relation,
	...
	public static Rel getRelationOfThatToMe(RelationParticipator that,
			RelationParticipator me) {
		return getRelationOfThatToMe(that, me, false);
	}

	public static Rel getRelationOfThatToMe(RelationParticipator that,
			RelationParticipator me, boolean ignorePeaceful) {
		Rel ret = null;

		Faction myFaction = getFaction(me);
		if (myFaction == null)
			return Rel.NEUTRAL; // ERROR
		...
		if (thatFaction.getRelationWish(myFaction).isLessThan(
				myFaction.getRelationWish(thatFaction))) {
		...
		} else if (!ignorePeaceful
				&& (thatFaction.getFlag(FFlag.PEACEFUL) || myFaction
						.getFlag(FFlag.PEACEFUL))) {
			ret = Rel.TRUCE;
		}
	...
	public static ChatColor getColorOfThatToMe(RelationParticipator that,
			RelationParticipator me) {
		Faction thatFaction = getFaction(that);
