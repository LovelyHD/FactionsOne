...
/* reference diagram, task should move in this pattern out from chunk 0 in the center. 8
 * [>][>][>][>][>] etc. [^][6][>][>][>][>][>][6] [^][^][4][>][>][>][4][v] [^][^][^][2][>][2][v][v]
 * [^][^][^][^][0][v][v][v] [^][^][^][1][1][v][v][v] [^][^][3][<][<][3][v][v]
 * [^][5][<][<][<][<][5][v] [7][<][<][<][<][<][<][7] */
public abstract class SpiralTask implements Runnable {
    ...
    // @SuppressWarnings("LeakingThisInConstructor") This actually triggers a
    // warning in Eclipse xD Could we find another way to suppress the error
    // please? :)
    public SpiralTask(FLocation fLocation, int radius) {
        ...
        setTaskID(Bukkit.getServer().getScheduler().scheduleSyncRepeatingTask(P.p, this, 2, 2));
    }

    /* This is where the necessary work is done; you'll need to override this method with whatever
	 * you want done at each chunk in the spiral pattern. Return false if the entire task needs to
	 * be aborted, otherwise return true to continue. */
    ...
    /* Returns a Location pointing at the current chunk X and Z values. note that the Location is at
	 * the corner of the chunk, not the center. */
