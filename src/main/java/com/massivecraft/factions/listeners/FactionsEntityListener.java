...
public class FactionsEntityListener implements Listener {
    ...
    public void onEntityDamage(EntityDamageEvent event) {
		 ...
		 * { // Players can not take any damage in a Safe Zone event.setCancelled(true); } */
    }
    ...
    public void onEntityExplode(EntityExplodeEvent event) {
        ...
        Set<FLocation> explosionLocs = new HashSet<>();
    ...
    private static final Set<PotionEffectType> badPotionEffects = new LinkedHashSet<>(Arrays.asList(PotionEffectType.BLINDNESS, PotionEffectType.CONFUSION, PotionEffectType.HARM,
            PotionEffectType.HUNGER, PotionEffectType.POISON, PotionEffectType.SLOW, PotionEffectType.SLOW_DIGGING, PotionEffectType.WEAKNESS, PotionEffectType.WITHER));
    ...
    public boolean canDamagerHurtDamagee(EntityDamageByEntityEvent sub, boolean notify) {
        ...
        /* // Damage will be dealt. However check if the damage should be reduced. if (damage > 0.0
		 * && defender.hasFaction() && ownTerritory && Conf.territoryShieldFactor > 0) { int
		 * newDamage = (int)Math.ceil(damage * (1D - Conf.territoryShieldFactor));
		 * sub.setDamage(newDamage);
		 * 
		 * // Send message if (notify) { String perc = MessageFormat.format("{0,number,#%}",
		 * (Conf.territoryShieldFactor)); // TODO does this display correctly??
    ...
    public void onEntityTarget(EntityTargetEvent event) {
        ...
        if (!Conf.monsters.contains(MiscUtil.creatureTypeFromEntity(event.getEntity()))) {
            return;
        }
