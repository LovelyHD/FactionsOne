...
public class FactionsEntityListener implements Listener {
	...
	public void onEntityDeath(EntityDeathEvent event) {
		...
		Faction faction = Board
				.getFactionAt(new FLocation(player.getLocation()));

		PowerLossEvent powerLossEvent = new PowerLossEvent(faction, fplayer);
		// Check for no power loss conditions
		if (!faction.getFlag(FFlag.POWERLOSS)) {
			powerLossEvent
					.setMessage("<i>You didn't lose any power since the territory you died in works that way.");
			powerLossEvent.setCancelled(true);
		} else if (Conf.worldsNoPowerLoss.contains(player.getWorld().getName())) {
			powerLossEvent
					.setMessage("<i>You didn't lose any power due to the world you died in.");
			powerLossEvent.setCancelled(true);
		} else {
		...
		if (msg != null && !msg.isEmpty()) {
			fplayer.msg(msg, fplayer.getPowerRounded(),
					fplayer.getPowerMaxRounded());
		}
	}
	...
	public void onEntityDamage(EntityDamageEvent event) {
		...
		/*
		 * else if (Conf.safeZonePreventAllDamageToPlayers &&
		 * isPlayerInSafeZone(event.getEntity())) { // Players can not take any
		 * damage in a Safe Zone event.setCancelled(true); }
		 */
	}
	...
	public void onEntityExplode(EntityExplodeEvent event) {
		...
		// TNT in water/lava doesn't normally destroy any surrounding blocks,
		// which is usually desired behavior, but...
		// this optional change below provides workaround for waterwalling
		// providing perfect protection,
		// and makes cheap (non-obsidian) TNT cannons require minor maintenance
		// between shots
		Block center = event.getLocation().getBlock();
		if (event.getEntity() instanceof TNTPrimed
				&& Conf.handleExploitTNTWaterlog && center.isLiquid()) {
			// a single surrounding block in all 6 directions is broken if the
			// material is weak enough
			...
			for (Block target : targets) {
				...
				// Bukkit yet
				if (id != 0 && (id < 7 || id > 11) && id != 49 && id != 90
						&& id != 116 && id != 119 && id != 120 && id != 130)
	...
	private static final Set<PotionEffectType> badPotionEffects = new LinkedHashSet<PotionEffectType>(
			Arrays.asList(PotionEffectType.BLINDNESS,
					PotionEffectType.CONFUSION, PotionEffectType.HARM,
	...
	public boolean canDamagerHurtDamagee(EntityDamageByEntityEvent sub,
			...
			if (damager instanceof Player) {
				if (notify) {
					...
					attacker.msg("<i>PVP is disabled in %s.",
							defLocFaction.describeTo(attacker));
				}
				return false;
			}
		...
		if (attacker.hasLoginPvpDisabled()) {
			if (notify)
				attacker.msg("<i>You can't hurt other players for "
						+ Conf.noPVPDamageToOthersForXSecondsAfterLogin
		...
		if (locFaction.getFlag(FFlag.PVP) == false) {
			if (notify)
				attacker.msg("<i>PVP is disabled in %s.",
						locFaction.describeTo(attacker));
			return false;
		}
		...
		if (attackFaction.isNone() && Conf.disablePVPForFactionlessPlayers) {
			...
			if (defLocFaction == attackFaction
					&& Conf.enablePVPAgainstFactionlessInAttackersLand) {
		...
		if (defender.hasFaction()
				&& relation.isAtLeast(Conf.friendlyFireFromRel)
				&& defLocFaction.getFlag(FFlag.FRIENDLYFIRE) == false) {
			if (notify)
				attacker.msg("<i>You can't hurt %s<i>.",
		...
		if (defender.hasFaction() && ownTerritory && relation == Rel.NEUTRAL) {
			if (notify) {
				attacker.msg(
						"<i>You can't hurt %s<i> in their own territory unless you declare them as an enemy.",
						defender.describeTo(attacker));
				defender.msg("%s<i> tried to hurt you.",
						attacker.describeTo(defender, true));
			}
			return false;
		}
		/*
		 * // Damage will be dealt. However check if the damage should be
		 * reduced. if (damage > 0.0 && defender.hasFaction() && ownTerritory &&
		 * Conf.territoryShieldFactor > 0) { int newDamage =
		 * (int)Math.ceil(damage * (1D - Conf.territoryShieldFactor));
		 * sub.setDamage(newDamage);
		 * 
		 * // Send message if (notify) { String perc =
		 * MessageFormat.format("{0,number,#%}", (Conf.territoryShieldFactor));
		 * // TODO does this display correctly??
	...
	public void onCreatureSpawn(CreatureSpawnEvent event) {
		...
		if (faction.getFlag(FFlag.MONSTERS))
			return;
		if (!Conf.monsters.contains(event.getEntityType()))
			return;
	...
	public void onEntityTarget(EntityTargetEvent event) {
		...
		if (!Conf.monsters.contains(MiscUtil.creatureTypeFromEntity(event
				.getEntity())))
			return;
	...
	public void onPaintingBreak(HangingBreakEvent event) {
		...
		if (event.getCause() == RemoveCause.EXPLOSION) {
			Faction faction = Board.getFactionAt(new FLocation(event
					.getEntity().getLocation()));
			if (faction.getFlag(FFlag.EXPLOSIONS) == false) { // faction has
																// explosions
																// disabled
				event.setCancelled(true);
				return;
		...
		if (!FactionsBlockListener.playerCanBuildDestroyBlock((Player) breaker,
				event.getEntity().getLocation().getBlock(), "remove paintings",
				false)) {
			event.setCancelled(true);
		}
	}
	...
	public void onPaintingPlace(HangingPlaceEvent event) {
		...
		if (!FactionsBlockListener.playerCanBuildDestroyBlock(
				event.getPlayer(), event.getBlock().getLocation().getBlock(),
				"place paintings", false)) {
			event.setCancelled(true);
		}
	}
