...
public class FactionsChatListener implements Listener {
	...
	static {
		try {
			fieldRegisteredListenerDotPriority = RegisteredListener.class
					.getDeclaredField("priority");
			fieldRegisteredListenerDotPriority.setAccessible(true);
		} catch (Exception e) {
			P.p.log(Level.SEVERE,
					"A reflection trick is broken! This will lead to glitchy relation-colored-chat.");
		}
	...
	/**
	 * At the Highest event priority we apply chat formating. Relation colored
	 * faction tags may or may not be disabled (Conf.chatParseTagsColored) If
	 * color is disabled it works flawlessly. If however color is enabled we
	 * face a limitation in Bukkit. Bukkit does not support the same message
	 * looking different for each recipient. The method we use to get around
	 * this is a bit hacky: 1. We cancel the chat event on EventPriority.HIGHEST
	 * 2. We trigger EventPriority.MONITOR manually without relation color. 3.
	 * We log in console the way it's usually done (as in nms.NetServerHandler
	 * line~793). 4. We send out the messages to each player with relation
	 * color. The side effect is that other plugins at EventPriority.HIGHEST may
	 * experience the event as cancelled.
	 */
	@EventHandler(priority = EventPriority.HIGHEST, ignoreCancelled = true)
	public void onPlayerChat(AsyncPlayerChatEvent event) {
		// Should we even parse?
		if (!Conf.chatParseTags)
			return;
		if (Conf.chatTagHandledByAnotherPlugin)
			return;
		...
		AsyncPlayerChatEvent monitorOnlyEvent = new AsyncPlayerChatEvent(false,
				from, message, new HashSet<Player>());
		monitorOnlyEvent.setFormat(formatWithoutColor);
		callEventAtMonitorOnly(monitorOnlyEvent);

		// 3. We log in console the way it's usually done (as in
		// nms.NetServerHandler line~793).
		Bukkit.getConsoleSender().sendMessage(
				String.format(monitorOnlyEvent.getFormat(), monitorOnlyEvent
						.getPlayer().getDisplayName(), monitorOnlyEvent
						.getMessage()));
		...
		for (Player to : event.getRecipients()) {
			...
			to.sendMessage(String.format(formatWithColor,
					from.getDisplayName(), message));
		}
	}

	/**
	 * This is some nasty woodo - I know :/ I should make a pull request to
	 * Bukkit and CraftBukkit to support this feature natively
	 */
	public static void callEventAtMonitorOnly(Event event) {
		synchronized (Bukkit.getPluginManager()) {
			...
			for (RegisteredListener registration : listeners) {
				try {
					EventPriority priority = (EventPriority) fieldRegisteredListenerDotPriority
							.get(registration);
	...
	public static String parseTags(String str, Player from, FPlayer fpfrom,
		...
		while (matcher.find()) {
			...
			matcher.appendReplacement(ret,
					produceTag(tag, args, from, fpfrom, to, fpto));
		}
	...
	public static String produceTag(String tag, List<String> args, Player from,
			FPlayer fpfrom, Player to, FPlayer fpto) {
		String ret = "";
