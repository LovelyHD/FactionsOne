...
public abstract class EntityCollection<E extends Entity> {
	...
	public EntityCollection(Class<E> entityClass, Collection<E> entities,
			Map<String, E> id2entity, File file, Gson gson, boolean creative) {
	...
	public EntityCollection(Class<E> entityClass, Collection<E> entities,
			Map<String, E> id2entity, File file, Gson gson) {
	...
	public E getBestIdMatch(String pattern) {
		String id = TextUtil.getBestStartWithCI(this.id2entity.keySet(),
				pattern);
		if (id == null)
			return null;
	...
	private Map<String, E> loadCore() {
		...
		if (type.toString().contains("FPlayer")) {
			...
			for (String value : data.keySet()) {
				if (!value
					...
					if (!value.matches("[a-zA-Z0-9_]{2,16}")) {
						// Not even a valid player name.. go ahead and mark it
						// for removal
			...
			if (list.size() > 0) {
				...
				Bukkit.getLogger()
						.log(Level.INFO,
								"Please wait while Factions converts "
										+ list.size()
										+ " old player names to UUID. This may take a while.");
				// First we'll make a backup, because god forbid anybody heed a
				// warning
				File file = new File(this.file.getParentFile(),
						"players.json.old");
				try {
					file.createNewFile();
				} catch (IOException e) {
				...
				Bukkit.getLogger().log(Level.INFO,
						"Backed up your old data at " + file.getAbsolutePath());
				...
				try {
					...
					for (String s : list) {
						...
						if (!response.containsKey(s)) {
							// They don't have a UUID so they should just be
							// removed
							invalidNames.add(s);
						}
					}
					for (String value : response.keySet()) {
						...
						if (player == null) {
							// The player never existed here, and shouldn't
							// persist
							invalidNames.add(value);
							continue;
						}
				...
				if (invalidNames.size() > 0) {
					...
					Bukkit.getLogger()
							.log(Level.INFO,
									"While converting we found names that either don't have a UUID or aren't players and removed them from storage.");
					Bukkit.getLogger().log(
							Level.INFO,
							"The following names were detected as being invalid: "
									+ StringUtils.join(invalidNames, ", "));
				}
		...
		try {
			...
			Bukkit.getLogger().log(
					Level.WARNING,
					"JSON error encountered loading \"" + file + "\": "
			...
			Bukkit.getLogger().log(Level.WARNING,
					"Backing up copy of bad file to: " + backup);
