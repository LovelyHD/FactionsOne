...
public class TextUtil {
	...
	public static final transient Pattern patternTag = Pattern
			.compile("<([a-zA-Z0-9_]*)>");
	...
	public static String parseColorTags(String string) {
		return string.replace("<empty>", "").replace("<black>", "\u00A70")
	...
	public static String implodeCommaAnd(List<String> list, String comma,
			String and) {
		if (list.size() == 0)
			return "";
		if (list.size() == 1)
	...
	public String titleize(String str) {
		...
		if (eatLeft < pivot)
			return parseTags("<a>")
					+ titleizeLine.substring(0, pivot - eatLeft) + center
					+ titleizeLine.substring(pivot + eatRight);
		else
			return parseTags("<a>") + center;
	}

	public ArrayList<String> getPage(List<String> lines, int pageHumanBased,
			String title) {
		...
		if (pagecount == 0) {
			...
			ret.add(this.parseTags("<i>Invalid page. Must be between 1 and "
					+ pagecount));
			return ret;
		}
	...
	public static String getBestStartWithCI(Collection<String> candidates,
			String start) {
		String ret = null;
		int best = 0;
