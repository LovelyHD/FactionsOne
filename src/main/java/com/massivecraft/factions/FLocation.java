...
public class FLocation {
	...
	// TODO: It would be wise to ask yourself is there is a better solution to
	// creating loads of new object
	...
	public Set<FLocation> getCircle(double radius) {
		...
		int xfrom = (int) Math.floor(x - radius);
		int xto = (int) Math.ceil(x + radius);
		int zfrom = (int) Math.floor(z - radius);
		int zto = (int) Math.ceil(z + radius);
		
		for (int x = xfrom; x <= xto; x++) {
			for (int z = zfrom; z <= zto; z++) {
				FLocation potential = new FLocation(worldName, x, z);
				if (getDistanceTo(potential) <= radius) {
					ret.add(potential);
				}
			}
		}
