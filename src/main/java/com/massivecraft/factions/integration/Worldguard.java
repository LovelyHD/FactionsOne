...
public class Worldguard {
	...
	public static void init(Plugin plugin) {
		Plugin wgplug = plugin.getServer().getPluginManager()
	...
	public static boolean checkForRegionsInChunk(Location loc) {
		...
		BlockVector maxChunk = new BlockVector(maxChunkX, worldHeight,
				maxChunkZ);

		RegionManager regionManager = wg.getRegionManager(world);
		ProtectedCuboidRegion region = new ProtectedCuboidRegion(
				"wgfactionoverlapcheck", minChunk, maxChunk);
		Map<String, ProtectedRegion> allregions = regionManager.getRegions();
		Collection<ProtectedRegion> allregionslist = new ArrayList<ProtectedRegion>(
				allregions.values());
