...
public class Econ {
	...
	public static void setup() {
		...
		String integrationFail = "Economy integration is "
				+ (Conf.econEnabled ? "enabled, but" : "disabled, and")
				+ " the plugin \"Vault\" ";
	...
	public static void sendBalanceInfo(FPlayer to, EconomyParticipator about) {
		if (!shouldBeUsed()) {
			P.p.log(Level.WARNING,
					"Vault does not appear to be hooked into an economy plugin.");
			return;
		}
		to.msg("<a>%s's<i> balance is <h>%s<i>.", about.describeTo(to, true),
				Econ.moneyString(econ.getBalance(about.getAccountId())));
	}
	...
	public static boolean canIControllYou(EconomyParticipator i,
		...
		// A faction can always transfer away the money of it's members and its
		// own money...
		// This will however probably never happen as a faction does not have
		// free will.
		...
		i.msg("<h>%s<i> lacks permission to control <h>%s's<i> money.",
				i.describeTo(i, true), you.describeTo(i));
		return false;
	}
	...
	public static boolean transferMoney(EconomyParticipator invoker,
		...
		if (isUUID(from.getAccountId())) {
			fromAcc = Bukkit.getOfflinePlayer(UUID.fromString(from
					.getAccountId()));
		} else {
		...
		if (!econ.has(fromAcc, amount)) {
			...
			if (invoker != null && notify)
				invoker.msg(
						"<h>%s<b> can't afford to transfer <h>%s<b> to %s<b>.",
						from.describeTo(invoker, true), moneyString(amount),
		...
		if (notify)
			invoker.msg("Unable to transfer %s<b> to <h>%s<b> from <h>%s<b>.",
					moneyString(amount), to.describeTo(invoker),
					from.describeTo(invoker, true));
	...
	public static void sendTransferInfo(EconomyParticipator invoker,
		...
		if (invoker == null) {
			for (FPlayer recipient : recipients) {
				recipient.msg(
						"<h>%s<i> was transfered from <h>%s<i> to <h>%s<i>.",
						moneyString(amount), from.describeTo(recipient),
						to.describeTo(recipient));
			}
		} else if (invoker == from) {
			for (FPlayer recipient : recipients) {
				recipient.msg("<h>%s<i> <h>gave %s<i> to <h>%s<i>.",
						from.describeTo(recipient, true), moneyString(amount),
						to.describeTo(recipient));
			}
		} else if (invoker == to) {
			for (FPlayer recipient : recipients) {
				recipient.msg("<h>%s<i> <h>took %s<i> from <h>%s<i>.",
						to.describeTo(recipient, true), moneyString(amount),
						from.describeTo(recipient));
			}
		} else {
			for (FPlayer recipient : recipients) {
				recipient
						.msg("<h>%s<i> transfered <h>%s<i> from <h>%s<i> to <h>%s<i>.",
								invoker.describeTo(recipient, true),
								moneyString(amount),
								from.describeTo(recipient),
								to.describeTo(recipient));
			}
	...
	public static boolean hasAtLeast(EconomyParticipator ep, double delta,
		...
		if (isUUID(ep.getAccountId())) {
			currentBalance = econ.getBalance(Bukkit.getOfflinePlayer(UUID
					.fromString(ep.getAccountId())));
		} else {
			currentBalance = econ.getBalance(Bukkit.getOfflinePlayer(ep
					.getAccountId()));
		}
		...
		if (!affordable) {
			if (toDoThis != null && !toDoThis.isEmpty())
				ep.msg("<h>%s<i> can't afford <h>%s<i> %s.",
						ep.describeTo(ep, true), moneyString(delta), toDoThis);
			return false;
		}
		return true;
	}
		...
		if (delta > 0) {
			...
			if (er.transactionSuccess()) {
				...
				if (forDoingThis != null && !forDoingThis.isEmpty())
					ep.msg("<h>%s<i> gained <h>%s<i> %s.", You,
							moneyString(delta), forDoingThis);
				return true;
			} else {
				// transfer to account failed
				if (forDoingThis != null && !forDoingThis.isEmpty())
					ep.msg("<h>%s<i> would have gained <h>%s<i> %s, but the deposit failed.",
							You, moneyString(delta), forDoingThis);
				return false;
			}
			...
			if (econ.has(acc, -delta)
					&& econ.withdrawPlayer(acc, -delta).transactionSuccess()) {
				// There is enough money to pay
				modifyUniverseMoney(-delta);
				if (forDoingThis != null && !forDoingThis.isEmpty())
					ep.msg("<h>%s<i> lost <h>%s<i> %s.", You,
							moneyString(-delta), forDoingThis);
				return true;
			} else {
				// There was not enough money to pay
				if (toDoThis != null && !toDoThis.isEmpty())
					ep.msg("<h>%s<i> can't afford <h>%s<i> %s.", You,
							moneyString(-delta), toDoThis);
				return false;
			}
		}
	}
	...
	public static double calculateClaimCost(int ownedLand,
		...
		// given for claiming from another faction
		return Conf.econCostClaimWilderness
				+ (Conf.econCostClaimWilderness
						* Conf.econClaimAdditionalMultiplier * ownedLand)
				- (takingFromAnotherFaction ? Conf.econCostClaimFromFactionBonus
						: 0);
	}

	// calculate refund amount for unclaiming land
	public static double calculateClaimRefund(int ownedLand) {
		return calculateClaimCost(ownedLand - 1, false)
				* Conf.econClaimRefundMultiplier;
	}
	...
	public static double calculateTotalLandRefund(int ownedLand) {
		return calculateTotalLandValue(ownedLand)
				* Conf.econClaimRefundMultiplier;
	}
	...
	public static boolean setBalance(String account, double amount) {
		...
		if (current > amount)
			return econ.withdrawPlayer(Bukkit.getOfflinePlayer(account),
					current - amount).transactionSuccess();
		else
			return econ.depositPlayer(Bukkit.getOfflinePlayer(account),
					amount - current).transactionSuccess();
	}
	...
	public static boolean modifyBalance(String account, double amount) {
		if (amount < 0)
			...
			return econ.depositPlayer(Bukkit.getOfflinePlayer(account), amount)
					.transactionSuccess();
	}
	...
	public static boolean withdraw(String account, double amount) {
		return econ.withdrawPlayer(Bukkit.getOfflinePlayer(account), amount)
				.transactionSuccess();
	}
