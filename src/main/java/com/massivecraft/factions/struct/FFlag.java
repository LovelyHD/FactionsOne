...
/**
 * Flags that describe the nature of a faction and it's territory. Can monsters
 * spawn there? May fire spread etc? Is the faction permanent? These flags have
 * nothing to do with player-permission.
 * 
 * The flags are either true or false.
 */
public enum FFlag {
	// Faction flags
	PERMANENT("permanent", "<i>A permanent faction will never be deleted.",
			false), PEACEFUL("peaceful",
			"<i>Allways in truce with other factions.", false), INFPOWER(
			"infpower", "<i>This flag gives the faction infinite power.", false),
	...
	POWERLOSS("powerloss", "<i>Is power lost on death in this territory?", true), PVP(
			"pvp", "<i>Can you PVP in territory?", true), FRIENDLYFIRE(
			"friendlyfire", "<i>Can friends hurt eachother here?", false), MONSTERS(
			"monsters", "<i>Can monsters spawn in this territory?", true), EXPLOSIONS(
			"explosions", "<i>Can explosions occur in this territory?", true), FIRESPREAD(
			"firespread", "<i>Can fire spread in territory?", true),
	...
	private FFlag(final String nicename, final String desc,
			final boolean defaultDefaultValue) {
	...
	public String getStateInfo(boolean value, boolean withDesc) {
		String ret = (value ? "<g>YES" : "<b>NOO") + "<c> "
				+ this.getNicename();
		if (withDesc) {
