	...
	public void perform() {
		...
		if (properFieldNames.isEmpty()) {
			...
			for (int i = 0; i < fields.length; i++) {
				properFieldNames.put(fields[i].getName().toLowerCase(),
						fields[i].getName());
			}
		}
		...
		try {
			...
			if (target.getType() == boolean.class) {
				...
				if (targetValue) {
					success = "\"" + fieldName
							+ "\" option set to true (enabled).";
				} else {
					success = "\"" + fieldName
							+ "\" option set to false (disabled).";
				}
			}
			...
			// int
				...
				try {
					...
					sendMessage("Cannot set \"" + fieldName
							+ "\": integer (whole number) value required.");
					return;
				}
			}
			...
			// long
				...
				try {
					...
					sendMessage("Cannot set \"" + fieldName
							+ "\": long integer (whole number) value required.");
					return;
				}
			}
			...
			// double
				...
				try {
					...
					success = "\"" + fieldName + "\" option set to "
							+ doubleVal + ".";
				} catch (NumberFormatException ex) {
					sendMessage("Cannot set \"" + fieldName
							+ "\": double (numeric) value required.");
					return;
				}
			}
			...
			// ChatColor
				...
				if (newColor == null) {
					sendMessage("Cannot set \"" + fieldName + "\": \""
							+ value.toUpperCase() + "\" is not a valid color.");
					return;
				}
				target.set(null, newColor);
				success = "\"" + fieldName + "\" color option set to \""
						+ value.toUpperCase() + "\".";
			}

			// Set<?> or other parameterized collection
			else if (target.getGenericType() instanceof ParameterizedType) {
				ParameterizedType targSet = (ParameterizedType) target
						.getGenericType();
				...
				if (targSet.getRawType() == Set.class) {
					...
					if (innerType == Material.class) {
						...
						if (newMat == null) {
							sendMessage("Cannot change \"" + fieldName
						...
						if (matSet.contains(newMat)) {
							...
							success = "\"" + fieldName + "\" set: Material \""
									+ value.toUpperCase() + "\" removed.";
						}
					...
					// Set of unknown type
						...
						sendMessage("\""
								+ fieldName
								+ "\" is not a data type set which can be modified with this command.");
						return;
					}
				}
				...
				// Map<?, ?>
					...
					if (args.size() < 3) {
						sendMessage("Cannot change \"" + fieldName
								+ "\" map: not enough arguments passed.");
						return;
					}
					...
					if (innerType == FFlag.class && innerType2 == Boolean.class) {
						...
						if (newFlag == null) {
							sendMessage("Cannot change \"" + fieldName
									+ "\" map: \"" + value1
									+ "\" is not a valid FFlag.");
							return;
						}

						@SuppressWarnings("unchecked")
						Map<FFlag, Boolean> map = (Map<FFlag, Boolean>) target
								.get(null);
					...
					// Map<FPerm, Set<Rel>>
					else if (innerType == FPerm.class
							&& innerType2 instanceof ParameterizedType) {
						if (((ParameterizedType) innerType2).getRawType() != Set.class) {
							sendMessage("\""
									+ fieldName
									+ "\" is not a data type map which can be modified with this command, due to the inner collection type.");
							return;
						}
						...
						@SuppressWarnings("unchecked")
						Map<FPerm, Set<Rel>> map = (Map<FPerm, Set<Rel>>) target
								.get(null);
					...
					// Map of unknown type
						...
						sendMessage("\""
								+ fieldName
								+ "\" is not a data type map which can be modified with this command.");
						return;
					}
				}
				...
				// not a Set or Map?
					...
					sendMessage("\""
							+ fieldName
							+ "\" is not a data collection type which can be modified with this command.");
					return;
				}
			}
			...
			// unknown type
				...
				sendMessage("\""
						+ fieldName
						+ "\" is not a data type which can be modified with this command.");
				return;
			}
		} catch (NoSuchFieldException ex) {
			sendMessage("Configuration setting \""
					+ fieldName
					+ "\" couldn't be matched, though it should be... please report this error.");
			return;
		} catch (IllegalAccessException ex) {
			sendMessage("Error setting configuration setting \"" + fieldName
					+ "\" to \"" + value + "\".");
			return;
		}
		...
		if (!success.isEmpty()) {
			if (sender instanceof Player) {
				...
				// using P.p.log() instead of sendMessage if run from server
				// console so that "[Factions v#.#.#]" is prepended in server
				// log
				P.p.log(success);
		}
		// save change to disk
