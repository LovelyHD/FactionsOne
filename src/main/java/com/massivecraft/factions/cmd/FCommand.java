...
public abstract class FCommand extends MCommand<P> {
	...
	public boolean validSenderType(CommandSender sender,
		...
		if (!(this.senderMustBeMember || this.senderMustBeOfficer || this.senderMustBeLeader))
			return true;
		...
		if (!fplayer.hasFaction()) {
			sender.sendMessage(p.txt
					.parse("<b>You are not member of any faction."));
			return false;
		}

		if (this.senderMustBeOfficer
				&& !fplayer.getRole().isAtLeast(Rel.OFFICER)) {
			sender.sendMessage(p.txt.parse(
					"<b>Only faction moderators can %s.", this.getHelpShort()));
			return false;
		}

		if (this.senderMustBeLeader && !fplayer.getRole().isAtLeast(Rel.LEADER)) {
			sender.sendMessage(p.txt.parse("<b>Only faction admins can %s.",
					this.getHelpShort()));
			return false;
		}
	...
	public Faction strAsFaction(String name, Faction def, boolean msg) {
		...
		if (msg && ret == null) {
			this.msg(
					"<b>The faction or player \"<p>%s<b>\" could not be found.",
					name);
		}
	...
	public FFlag strAsFactionFlag(String name, FFlag def, boolean msg) {
		...
		if (msg && ret == null) {
			this.msg("<b>The faction-flag \"<p>%s<b>\" could not be found.",
					name);
		}
	...
	public FPerm strAsFactionPerm(String name, FPerm def, boolean msg) {
		...
		if (msg && ret == null) {
			this.msg("<b>The faction-perm \"<p>%s<b>\" could not be found.",
					name);
		}
	...
	public boolean canIAdministerYou(FPlayer i, FPlayer you) {
		if (!i.getFaction().equals(you.getFaction())) {
			i.sendMessage(p.txt.parse(
					"%s <b>is not in the same faction as you.",
					you.describeTo(i, true)));
			return false;
		}

		if (i.getRole().isMoreThan(you.getRole())
				|| i.getRole().equals(Rel.LEADER)) {
			return true;
		}
		...
		if (you.getRole().equals(Rel.LEADER)) {
			...
			if (i == you) {
				...
				i.sendMessage(p.txt
						.parse("<b>Moderators can't control each other..."));
			}
		} else {
			i.sendMessage(p.txt
					.parse("<b>You must be a faction moderator to do that."));
		}
	...
	public boolean payForCommand(double cost, String toDoThis,
			String forDoingThis) {
		if (!Econ.shouldBeUsed() || this.fme == null || cost == 0.0
				|| fme.hasAdminMode())
			return true;
