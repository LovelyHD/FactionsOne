...
public abstract class FCommand extends MCommand<P> {
	...
	public boolean validSenderType(CommandSender sender, boolean informSenderIfNot) {
		...
		if ( !(senderMustBeMember || senderMustBeOfficer || senderMustBeLeader)) {
			return true;
		}
		...
		if (senderMustBeOfficer && !fplayer.getRole().isAtLeast(Rel.OFFICER)) {
			sender.sendMessage(p.txt.parse("<b>Only faction moderators can %s.", getHelpShort()));
			return false;
		}
		
		if (senderMustBeLeader && !fplayer.getRole().isAtLeast(Rel.LEADER)) {
			sender.sendMessage(p.txt.parse("<b>Only faction admins can %s.", getHelpShort()));
			return false;
		}
	...
	public boolean assertMinRole(Rel role) {
		...
		if (fme.getRole().isLessThan(role)) {
			msg("<b>You <h>must be " + role + "<b> to " + getHelpShort() + ".");
			return false;
		}
		return true;
	}
	...
	public FPlayer strAsFPlayer(String name, FPlayer def, boolean msg) {
		...
		if (msg && ret == null) {
			msg("<b>No player \"<p>%s<b>\" could be found.", name);
		}
	...
	public FPlayer argAsFPlayer(int idx, FPlayer def, boolean msg) {
		return strAsFPlayer(this.argAsString(idx), def, msg);
	}
	...
	public FPlayer argAsBestFPlayerMatch(int idx, FPlayer def, boolean msg) {
		return strAsBestFPlayerMatch(this.argAsString(idx), def, msg);
	}
	...
	public Faction strAsFaction(String name, Faction def, boolean msg) {
		...
		if (msg && ret == null) {
			msg("<b>The faction or player \"<p>%s<b>\" could not be found.", name);
		}
	...
	public Faction argAsFaction(int idx, Faction def, boolean msg) {
		return strAsFaction(this.argAsString(idx), def, msg);
	}
	...
	public FFlag strAsFactionFlag(String name, FFlag def, boolean msg) {
		...
		if (msg && ret == null) {
			msg("<b>The faction-flag \"<p>%s<b>\" could not be found.", name);
		}
	...
	public FFlag argAsFactionFlag(int idx, FFlag def, boolean msg) {
		return strAsFactionFlag(this.argAsString(idx), def, msg);
	}
	...
	public FPerm strAsFactionPerm(String name, FPerm def, boolean msg) {
		...
		if (msg && ret == null) {
			msg("<b>The faction-perm \"<p>%s<b>\" could not be found.", name);
		}
	...
	public FPerm argAsFactionPerm(int idx, FPerm def, boolean msg) {
		return strAsFactionPerm(this.argAsString(idx), def, msg);
	}
	...
	public Rel strAsRel(String name, Rel def, boolean msg) {
		...
		if (msg && ret == null) {
			msg("<b>The role \"<p>%s<b>\" could not be found.", name);
		}
	...
	public Rel argAsRel(int idx, Rel def, boolean msg) {
		return strAsRel(this.argAsString(idx), def, msg);
	}
	...
	public boolean payForCommand(double cost, String toDoThis, String forDoingThis) {
		if ( !Econ.shouldBeUsed() || fme == null || cost == 0.0 || fme.hasAdminMode()) {
			return true;
		}
