...
public class CmdList extends FCommand {
	...
	public void perform() {
		...
		if (!payForCommand(Conf.econCostList, "to list the factions",
				"for listing the factions"))
			return;
		 ...
		 * every faction and then only showing 9 of them; rewritten below
		 * lines.add(p.txt.parse("<i>Factionless<i> %d online",
		 * Factions.i.getNone().getFPlayersWhereOnline(true).size())); for
		 * (Faction faction : factionList) {
		...
		lines.add(p.txt
				.titleize("Faction List " + pagenumber + "/" + pagecount));

		for (Faction faction : factionList.subList(start, end)) {
			if (faction.isNone()) {
				lines.add(p.txt.parse("<i>Factionless<i> %d online", Factions.i
						.getNone().getFPlayersWhereOnline(true).size()));
				continue;
			}
			lines.add(p.txt.parse("%s<i> %d/%d online, %d/%d/%d", faction
					.getTag(fme), faction.getFPlayersWhereOnline(true).size(),
					faction.getFPlayers().size(), faction.getLandRounded(),
					faction.getPowerRounded(), faction.getPowerMaxRounded()));
		}
