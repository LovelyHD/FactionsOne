...
public class CmdCreate extends FCommand {
	...
	public void perform() {
		...
		if (!payForCommand(Conf.econCostCreate, "to create a new faction",
		...
		FPlayerJoinEvent joinEvent = new FPlayerJoinEvent(FPlayers.i.get(me),
				faction, FPlayerJoinEvent.PlayerJoinReason.CREATE);
		...
		for (FPlayer follower : FPlayers.i.getOnline()) {
			follower.msg("%s<i> created a new faction %s",
					fme.describeTo(follower, true), faction.getTag(follower));
		}

		msg("<i>You should now: %s",
				p.cmdBase.cmdDescription.getUseageTemplate());
