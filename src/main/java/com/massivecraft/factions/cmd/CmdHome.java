	...
	public void perform() {
		...
		if (!myFaction.hasHome()) {
			fme.msg("<b>Your faction does not have a home. "
					+ (fme.getRole().isLessThan(Rel.OFFICER) ? "<i> Ask your leader to:"
							: "<i>You should:"));
			fme.sendMessage(p.cmdBase.cmdSethome.getUseageTemplate());
			return;
		}

		if (!Conf.homesTeleportAllowedFromEnemyTerritory
				&& fme.isInEnemyTerritory()) {
		...
		if (!Conf.homesTeleportAllowedFromDifferentWorld
				&& me.getWorld().getUID() != myFaction.getHome().getWorld()
						.getUID()) {
		...
		if (Conf.homesTeleportAllowedEnemyDistance > 0
			...
			for (Player p : me.getServer().getOnlinePlayers()) {
				if (p == null || !p.isOnline() || p.isDead() || p == me
						|| p.getWorld() != w)
					continue;
				...
				fme.msg("<b>You cannot teleport to your faction home while an enemy is within "
		...
		if (!payForCommand(Conf.econCostHome,
				...
				"for teleporting to your faction home"))
			return;
