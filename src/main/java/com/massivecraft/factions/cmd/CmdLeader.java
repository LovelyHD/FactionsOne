...
public class CmdLeader extends FCommand {
    ...
    public CmdLeader() {
        ...
        permission = Permission.LEADER.node;
        disableOnLock = true;
    ...
    public void perform() {
        ...
        if (senderIsConsole || fme.hasAdminMode() || Permission.LEADER_ANY.has(sender, false)) {
            // Do whatever you wish
        } else {
            // Follow the standard rules
            if (fme.getRole() != Rel.LEADER || targetFaction != myFaction) {
                sender.sendMessage(p.txt.parse("<b>You must be leader of the faction to %s.", getHelpShort()));
                return;
            }
        ...
        for (FPlayer fplayer : FPlayers.i.getOnline()) {
            fplayer.msg("%s<i> gave %s<i> the leadership of %s<i>.", senderIsConsole ? "A server admin" : RelationUtil.describeThatToMe(fme, fplayer, true), newLeader.describeTo(fplayer),
