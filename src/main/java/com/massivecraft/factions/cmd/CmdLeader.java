...
public class CmdLeader extends FCommand {
	...
	public void perform() {
		...
		if (this.senderIsConsole || fme.hasAdminMode()
				|| Permission.LEADER_ANY.has(sender, false)) {
			// Do whatever you wish
		} else {
			// Follow the standard rules
			if (fme.getRole() != Rel.LEADER || targetFaction != myFaction) {
				sender.sendMessage(p.txt.parse(
						"<b>You must be leader of the faction to %s.",
						this.getHelpShort()));
				return;
			}

			if (newLeader.getFaction() != myFaction) {
				msg("%s<i> is not a member in the faction.",
						newLeader.describeTo(fme, true));
				return;
			}
		...
		if (newLeader.getFaction() != targetFaction) {
			FPlayerJoinEvent event = new FPlayerJoinEvent(FPlayers.i.get(me),
					targetFaction, FPlayerJoinEvent.PlayerJoinReason.LEADER);
		...
		if (targetFactionCurrentLeader == newLeader) {
			...
			msg("<i>You have demoted %s<i> from the position of faction leader.",
					newLeader.describeTo(fme, true));
			newLeader
					.msg("<i>You have been demoted from the position of faction leader by %s<i>.",
							senderIsConsole ? "a server admin" : fme
									.describeTo(newLeader, true));
			return;
		}
		...
		msg("<i>You have promoted %s<i> to the position of faction leader.",
				newLeader.describeTo(fme, true));
		...
		for (FPlayer fplayer : FPlayers.i.getOnline()) {
			fplayer.msg(
					...
					senderIsConsole ? "A server admin" : RelationUtil
							.describeThatToMe(fme, fplayer, true), newLeader
							.describeTo(fplayer), targetFaction
							.describeTo(fplayer));
		}
	}
}
