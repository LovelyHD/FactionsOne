...
public class Faction extends Entity implements EconomyParticipator {
	...
	public void confirmValidHome() {
		if (!Conf.homesMustBeInClaimedTerritory
				|| this.home == null
				|| (this.home.getLocation() != null && Board
						.getFactionAt(new FLocation(this.home.getLocation())) == this))
			return;
	...
	public Map<Rel, List<String>> getFactionTagsPerRelation(
			RelationParticipator rp, boolean onlyNonNeutral) {
	...
	public void promoteNewLeader() {
		...
		if (this.getFlag(FFlag.PERMANENT)
				&& Conf.permanentFactionsDisableLeaderPromotion)
			return;
		...
		// officers
		ArrayList<FPlayer> replacements = this
				.getFPlayersWhereRole(Rel.OFFICER);
		...
		if (replacements == null || replacements.isEmpty()) { // faction leader
			...
			for (FPlayer fplayer : FPlayers.i.getOnline()) {
				fplayer.msg("The faction %s<i> was disbanded.",
						this.getTag(fplayer));
			}
			...
			this.msg(
					...
					oldLeader == null ? "" : oldLeader.getName(), replacements
							.get(0).getName());
			P.p.log("Faction " + this.getTag() + " (" + this.getId()
	...
	public boolean hasOfflineExplosionProtection() {
		...
		long timeUntilNoboom = this.getLastOnlineTime()
				+ (long) (Conf.offlineExplosionProtectionDelay * 60 * 1000);
